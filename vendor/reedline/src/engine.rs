        ...
        match event {
            ...
            ReedlineEvent::Edit(commands) => {
                ...
                if let Some(menu) = self.menus.iter_mut().find(|men| men.is_active()) {
                    ...
                    // Typing a word boundary ends the completion the menu was
...
mod tests {
    ...
    fn enter_with_an_empty_menu_submits_the_line() {
        ...
        reedline
            .handle_event(&prompt, ReedlineEvent::Edit(vec![EditCommand::InsertChar('z')]))
            .unwrap();
    ...
    /// types the rest of the statement and hits Enter. Before the fix the
    /// menu's highlighted suggestion was inserted at the cursor instead of
    /// the line running.
    #[test]
