...
impl TextElement {
    ...
    fn layout_cursor(
            ...
            (cursor_pos, cursor_start, cursor_end)
        {
            ...
            // DuckTable patch: a DISABLED editor never scrolls itself —
            // selection changes must not adjust the scroll offset (this
            // block writes it directly, bypassing the range clamp).
            if selection_changed && !is_selected_all && !state.disabled {
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        // DuckTable patch: a DISABLED editor never scrolls itself. No
        // scroll-past-end room, so a content-sized read-only card has a
        // zero scroll range and the wheel falls through to the page.
