...
bundles the build it was made with. `DUCKDB_LIB_BUILD` names a build
(`alpha42289`) whose library comes from the `engine-<build>` release of
this repository in place of the channel, for the local fetch, CI and the
release builds alike, when the channel moves to an engine harbor cannot
load; `latest` is the channel. (For a
